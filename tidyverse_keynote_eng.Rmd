...
# So what's exaclty *in* the tidyverse?
...
* `readr` a fast and friendly way to read rectangular data (like csv, tsv, etc.).

* `tibble` a tibble is a modern re-imagining of the data frame, keeping what time has proven to be effective, and throwing out what it has not.
...
n_parcelas_sep <-  separate(n_parcelas_tidy, IFN, c('name', 'ifn_version'), sep = '_')
n_parcelas_sep
...
```{r unite_ex}
n_parcelas_unite <- unite(n_parcelas_sep, IFN, name, ifn_version, sep = '_')
n_parcelas_unite
```
