---
title: "Introduction to the tidyverse: `dplyr`, `tidyr` and `purrr`"
...
Thi is a `RMarkdown` document generated to illustrate the exercices suggested as part of the Workshop "Introduction to the tidyverse", that will be celebrated in Barcelona within the 1st Meeting of the Iberian Ecological Society and the XIV AEET Meeting. All the code and the data needed to produce this document can be found in GitHub ([https://github.com/ameztegui/dplyr_workshop](https://github.com/ameztegui/dplyr_workshop)). For any doubt about the exercises, you can contact Aitor Ameztegui ([ameztegui@gmail.com](ameztegui@gmail.com)) or Víctor Granda  ([ameztegui@gmail.com](ameztegui@gmail.com)).

The packages` tidyr`, `dplyr`, and `purrr` are part of a set of packages known as the  [`tidyverse`](https://cran.r-project.org/web/packages/tidyverse/index.html), created by [Hadley Wickham](https://github.com/hadley), Chief Scientist in RStudio. The `tidyverse` was created to easen data analysis and data management. It consists on packages to import and read data, to organize and modify them, to analyze and model them and to visualize them. In this seminar we will focus on `tidyr`, conceived to help organize data, and `dplyr`, that focuses on data transformation. We must firs install these packages (if we haven't done it yet). To install all the packages from the `tidyverse` at once we  need to write  ```install.packages("tidyverse")```. Then we need to load the packages by typing:

```{r, results="hide", message=F, warning=FALSE,}
library(tidyverse)
```

We also need to load the dataset we will use for this workshop, we can download them from  [GitHub](https://github.com/ameztegui/dplyr_workshop).
...
`dplyr` can be used to transform our data frames in the way we need it in each case: we can create bew variables, select those of interest, execute filters, etc. The `dplyr` package contains 5 main verbs:
...
```{r, results="hide"}
trees <- mutate(trees, BAIf2= (((pi/4)*(DiamIf2/100)^2)*Fac),
...
In the case of our data frame `trees`, there are several groups that could be of interest el caso de nuestra base de datos de pies trees, hay varios grupos que pueden tener interés:
...
trees %>%
    mutate(growth=DiamIf3-DiamIf2) %>%
    group_by(Codi) %>%
    mutate(des = (growth - mean(growth))/sd(growth)) %>%
    arrange(desc(des))
```
...
Sometimes, none of the verbes implemented in `dplyr` will do what we want to perform, and we will need to implement different functions, either existent or created by us. This can be done with the `do` function. The `do` function is much slower than the verbs we've seen so far, but it allows to implement any function at all. Since the function we will introduce in `do` does not necessarily understand the pipelines (`%>%`) we will use a pronoun  (`.`) to specify that the function is applied to the group defined previously. So, in the same way that `head` is giving us the first n rows of a data frame

```{r Ejemplo do 1}
head(trees, 5)
```

We could use `do` to obtaine the first five lines of *each* plot, or each species:
...
Or we could even use it to adjust a model to each group. In this case, we will apply a linear model to each species, relating height and diameter, but we could use any function created by us:
...
In this case, the output contains a column, named `mod` (the name was defined by us), which is in reality a list. This is a bit weird, but since columns in a data frame can contain any type of data, nothing prevents them of containing lists. Each of the elements of the list conrains the output of the lineal model fit for that specues. If we want to obtain, for example, the coefficients for Pinus sylvestris (Especie =="021"), we just need to type:
...
* (C) Define the variables we DO NOT want to include (with -). The function will assume we want to gather the rest of variables.
